...
pub struct Config {
    ...
    pub workspace_order_file: PathBuf,
    pub ssh_destinations_file: PathBuf,
    pub snapshot_lock: PathBuf,
    pub workspace_order_lock: PathBuf,
    pub status_lock: PathBuf,
...
impl Config {
    pub fn from_env() -> Result<Self> {
        ...
        Ok(Self {
            ...
            workspace_order_file: state_root.join("workspace.order"),
            ssh_destinations_file: state_root.join("ssh-destinations"),
            snapshot_lock: state_root.join(".snapshot.lock"),
            workspace_order_lock: state_root.join(".workspace-order.lock"),
            status_lock: state_root.join(".status.lock"),
